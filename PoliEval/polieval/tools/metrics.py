...
class AccuracyThresh(Metric):
    ...
    def value(self):
        ...
        # print(1111,self.y_pred>self.thresh, self.y_true.byte(),self.y_pred[0],self.y_true.byte()[0])
...
class F1Score(Metric):
    ...
    def __call__(self,logits,target):
        ...
        if self.normalizate and self.task_type == 'binary':
            y_prob = torch.sigmoid(logits).data.cpu().numpy()
        elif self.normalizate and self.task_type == 'multiclass':
            y_prob = torch.softmax(logits,-1).data.cpu().detach().numpy()
        else:
